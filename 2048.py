...
class gameBoard:
    ...
    def addDown(self, Y):
        ...
        for X in reversed(range(0, 3)):
            if self.board[X + 1][Y] is self.board[X][Y] \
                and self.board[X][Y] is not None:
    ...
    def addLeft(self, X):
        added = False
        for Y in range(0, 3):
            if self.board[X][Y + 1] is self.board[X][Y] \
                and self.board[X][Y] is not None:
    ...
    def addUp(self, Y):
        added = False
        for X in range(0, 3):
            if self.board[X][Y] is self.board[X + 1][Y] \
                and self.board[X][Y] is not None:
                self.increaseValue(X, Y)
        ...
        # IF ID IS 1 IT IS THE TURN OF THE AI
            ...
            for i in range(0, 4):
                ...
                if newTemp.moveBoard(i):
                    score = max(score, self.expectimax(depth - 1, 0,
                                newTemp))
                    moveBool = True
...
def findTiles():
    region = (Game_Region[0] + boarder, Game_Region[1] + boarder,
              square, square)
    n = 0
    global tiles
    tiles = [[0 for x in range(4)] for y in range(4)]
    tiles[0][0] = tile(Game_Region[0] + boarder, Game_Region[1]
                       + boarder)
...
def checkSquare(x, y):
    ...
    # COLOR FOR 2
    if pyautogui.pixelMatchesColor(tiles[x][y].x + 20, tiles[x][y].y
                                   + 20, (239, 219, 185)):
        game.setValue(x, y, 2)
        return True

    # COLOR FOR 4
    elif pyautogui.pixelMatchesColor(tiles[x][y].x + 20, tiles[x][y].y
            + 20, (239, 215, 170)):
        game.setValue(x, y, 4)
        return True
